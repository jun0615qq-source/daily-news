...
export default function Login() {
  ...
  async function handleReset() {
    if (!form.email) { setError('비밀번호 재설정을 위해 이메일을 입력하세요.'); return; }
    try {
        ...
        <div className="bg-white dark:bg-navy-800 rounded-3xl shadow-xl p-8 border border-beige-200 dark:border-navy-700 animate-slide-up">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl
                       ...
                       hover:bg-beige-50 dark:hover:bg-navy-700 transition-colors mb-5
                       disabled:opacity-50"
          >
          ...
          <div className="relative mb-5">
            ...
            <div className="relative flex justify-center">
              <span className="px-3 text-xs text-gray-400 dark:text-gray-500 bg-white dark:bg-navy-800">또는 이메일로 로그인</span>
            </div>
          </div>
          ...
          {resetSent && (
            <div className="mb-4 p-3 rounded-xl bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
              <p className="text-sm text-green-600 dark:text-green-400">비밀번호 재설정 이메일을 전송했습니다.</p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <div className="flex justify-end">
              <button type="button" onClick={handleReset}
                className="text-xs text-beige-600 dark:text-navy-300 hover:underline">
                비밀번호를 잊으셨나요?
              </button>
            </div>
